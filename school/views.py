...
def afterlogin_view(request):
    if is_admin(request.user):
        ...
        accountapproval=models.TeacherExtra_1.objects.all().filter(user_id=request.user.id,status=True)
        if accountapproval:
            return redirect('teacher-dashboard')
        else:
            return render(request,'school/teacher_wait_for_approval.html')
    elif is_student(request.user):
        accountapproval=models.StudentExtra_1.objects.all().filter(user_id=request.user.id,status=True)
        if accountapproval:
...
def admin_dashboard_view(request):
    teachercount=models.TeacherExtra_1.objects.all().filter(status=True).count()
    pendingteachercount=models.TeacherExtra_1.objects.all().filter(status=False).count()

    studentcount=models.StudentExtra_1.objects.all().filter(status=True).count()
    pendingstudentcount=models.StudentExtra_1.objects.all().filter(status=False).count()

    teachersalary=models.TeacherExtra_1.objects.filter(status=True).aggregate(Sum('salary'))
    pendingteachersalary=models.TeacherExtra_1.objects.filter(status=False).aggregate(Sum('salary'))

    studentfee=models.StudentExtra_1.objects.filter(status=True).aggregate(Sum('fee_paid',default=0))
    pendingstudentfee=models.StudentExtra_1.objects.filter(status=False).aggregate(Sum('fee_paid'))
    ...
    mydict={
        ...
        'studentfee':studentfee['fee_paid__sum'],
        'pendingstudentfee':pendingstudentfee['fee_paid__sum'],
...
def admin_view_teacher_view(request):
    teachers=models.TeacherExtra_1.objects.all().filter(status=True)
...
def admin_approve_teacher_view(request):
    teachers=models.TeacherExtra_1.objects.all().filter(status=False)
...
def approve_teacher_view(request,pk):
    teacher=models.TeacherExtra_1.objects.get(id=pk)
    teacher.status=True
    teacher.save()
...
def admin_view_teacher_salary_view(request):
    teachers=models.TeacherExtra_1.objects.all()
...
def admin_view_student_view(request):
    students=models.StudentExtra_1.objects.all().filter(status=True)
...
def delete_student_from_school_view(request,pk):
    student=models.StudentExtra_1.objects.get(id=pk)
...
def admin_approve_student_view(request):
    students=models.StudentExtra_1.objects.all().filter(status=False)
...
def approve_student_view(request,pk):
    students=models.StudentExtra_1.objects.get(id=pk)
...
def admin_view_student_fee_view(request):
    students=models.StudentExtra_1.objects.all()
...
def admin_take_attendance_view(request,cl):
    students=models.StudentExtra_1.objects.all().filter(cl=cl)
    print(students)
...
def admin_view_attendance_view(request,cl):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            studentdata=models.StudentExtra_1.objects.all().filter(cl=cl)
...
def admin_view_fee_view(request,cl):
    feedetails=models.StudentExtra_1.objects.all().filter(cl=cl)
...
#notice related viewssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss
@login_required(login_url='adminlogin')
@user_passes_test(is_admin)
...
def teacher_dashboard_view(request):
    teacherdata=models.TeacherExtra_1.objects.all().filter(status=True,user_id=request.user.id)
    notice=models.Notice.objects.all()
    mydict={
...
def student_dashboard_view(request):
    studentdata=models.StudentExtra_1.objects.all().filter(status=True,user_id=request.user.id)
    notice=models.Notice.objects.all()
    mydict={
        'class':studentdata[0].cl,
        'mobile':studentdata[0].personal_mobile,
        'fee':studentdata[0].fee_paid,
        'notice':notice
    }
...
def student_attendance_view(request):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            date=form.cleaned_data['date']
            studentdata=models.StudentExtra_1.objects.all().filter(user_id=request.user.id,status=True)
